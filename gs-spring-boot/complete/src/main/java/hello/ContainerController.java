...
public class ContainerController {
	String key = "Endpoint=sb://containertracetrack.servicebus.windows.net/;SharedAccessKeyName=Hub;SharedAccessKey=NtCPpCZVlCLKovQ0wb/fvFK906itu/F0Q+V1DZRY7V0=;EntityPath=trackingdata";
     final Map<String,String> countryCodeMap = new HashMap<String,String>();
    
	

	@RequestMapping(value = "/containerTracking", method = RequestMethod.POST, consumes = "application/json", produces = "application/json")
	public @ResponseBody String containerTraceAndTrack(@RequestBody Container containerData) throws EventHubException, IOException {
		...
		final EventHubClient ehClient = EventHubClient.createFromConnectionStringSync(key, executorService);
		...
		return "Container tracking data is successfully pushed into Eventhub";
			...
			while (true) {
				...
				if (receivedEvents != null) {
					for (final EventData receivedEvent : receivedEvents) {
						...
						containerData = g.fromJson(new String(receivedEvent.getBytes(), Charset.defaultCharset()),
								Container.class);
...
// Quota limitation on maximum number of concurrent receivers per consumergroup per partition is 5
	...
	@RequestMapping(value = "/jsonToxml", method = RequestMethod.POST, consumes = "application/json", produces = "application/xml")
	public @ResponseBody String jsonToxml(@RequestBody Container containerData) {
		...
		try {
			...
			System.out.println(": Send Complete..." + xml_data);
			return xml_data;
		} finally {
	...
	@RequestMapping(value = "/xmlToJson", method = RequestMethod.POST, consumes = "application/xml", produces = "application/json")
	public @ResponseBody String xmlToJson(@RequestBody Container containerData) {
		...
		try {
			...
			obj.toString();
			System.out.println(": Send Complete..." + containerData.getConsignee());
			return obj.toString();
		} finally {
	...
	public @ResponseBody String countryCode(@RequestBody Country countryCode) {
		...
		try {
			...
			countryCodeMap.put(countryCode.getCountryName(), countryCode.getCountryCode());
	...
	public @ResponseBody String getcountryCode(@RequestParam("countryname") String name) {

		try {
			System.out.println("name"+name);
			String code=countryCodeMap.get(name);
